...
const PenyewaDashboard = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        setLoading(true)
        const [complaints, invoices, room, rental] = await Promise.all([
          dashboardService.getComplaints(),
          dashboardService.getInvoices(),
          dashboardService.getMyRoom(),
          dashboardService.getMyRentalInfo(),
        ])
        
        // Get pending payments count (unpaid or overdue invoices)
        const pendingCount = invoices?.filter(inv => inv.status === 'unpaid' || inv.status === 'overdue')?.length || 0
        ...
        setStats({
          ...
          rentEndDate: rental?.current_end_date || null,
        })
      } catch (error) {
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                  ...
                  <div className="text-right">
                    ...
                    {stats.rentEndDate && (
                      <div className="bg-green-500/30 rounded-lg px-4 py-2">
                        <p className="text-sm opacity-75">Sewa Sampai</p>
                        <p className="text-lg font-bold">
                          {new Date(stats.rentEndDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
                        </p>
                      </div>
                    )}
                  </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition-shadow border border-gray-100">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-gray-500 font-medium mb-2">Periode Sewa Berakhir</h3>
                    <p className="text-2xl font-bold text-blue-600">
                      {stats.rentEndDate 
                        ? new Date(stats.rentEndDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })
                        : '-'
                      }
                    </p>
                    <p className="text-sm text-gray-400 mt-2">
                      {stats.rentEndDate 
                        ? `${Math.ceil((new Date(stats.rentEndDate) - new Date()) / (1000 * 60 * 60 * 24))} hari lagi`
                        : 'Belum ada data'
                      }
                    </p>
                  </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <button
                  onClick={() => navigate('/penyewa/fitur/rfid')}
                ...
                >
                  <span className="text-sm font-semibold text-gray-700">TAP RFID</span>
                </button>
              </div>
            </div>
          </div>
