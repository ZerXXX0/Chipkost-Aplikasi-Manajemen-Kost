...
class RFIDCardViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return RFIDCard.objects.filter(tenant=user).order_by('-registered_at')
    
    def perform_create(self, serializer):
        serializer.save(registered_by=self.request.user)
    
    @action(detail=False, methods=['post'], url_path='tap', permission_classes=[AllowAny])
    def tap_card(self, request):
        ...
        try:
            rfid_card = RFIDCard.objects.get(card_uid=card_id)
        ...
        if rfid_card.tenant:
            # Notif model expects 'pesan' and 'status' fields
            Notif.objects.create(
                user=rfid_card.tenant,
                pesan=f'Selamat datang di Kamar {room.room_number}',
                status='unread'
            )
        ...
        response_data = {
            ...
            'card_id': card_id,
            'penyewa': rfid_card.tenant.username if rfid_card.tenant else None,
            'tenant_name': f"{rfid_card.tenant.first_name} {rfid_card.tenant.last_name}".strip() if rfid_card.tenant else None,
            'room_number': room.room_number,
            'kos_name': room.kos.name if room.kos else 'Unknown Kos',
            'room_display': f"Kamar {room.room_number} - {room.kos.name if room.kos else 'Unknown Kos'}",
...
class AccessLogViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == 'penyewa':
            queryset = queryset.filter(rfid_card__tenant=user)
