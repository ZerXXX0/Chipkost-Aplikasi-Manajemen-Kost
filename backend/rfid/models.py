...
class RFIDCard(models.Model):
    ...
    card_uid = models.CharField(max_length=50, unique=True, db_column='card_uid')
    tenant = models.ForeignKey(User, on_delete=models.SET_NULL, related_name='rfid_cards', limit_choices_to={'role': 'penyewa'}, db_column='tenant_id', null=True, blank=True)
    ...
    # Property untuk backward compatibility dengan API
    @property
    def card_id(self):
    ...
    def __str__(self):
        username = self.tenant.username if self.tenant else 'No tenant'
        return f"{self.card_uid} - {username}"
